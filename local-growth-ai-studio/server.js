...
const ledger = new Ledger(process.env.CREDIT_DB || join(ROOT, 'data', 'credits.sqlite'));
...
function validateInput(body) {
  ...
  return { mode: body.mode, language: body.language, productName: text(body.productName,120,'Product name'), description: text(body.description,1800,'Description'), audience: text(body.audience,240,'Audience'), imageDataUrl };
}
...
async function api(req, res, url) {
  if (!rateLimit(req, url.pathname)) { res.setHeader('Retry-After','60'); return json(res,429,{ok:false,error:'Too many requests. Wait one minute.'}); }
  if (req.method === 'POST') {
  ...
  if (req.method === 'GET' && url.pathname === '/api/health') return json(res,200,{
    ok:true, engine:config.liveEnabled ? 'openai' : config.allowDemo ? 'demo' : 'disabled',
    aiConnected:config.liveEnabled, videoProviderConnected:false,
  ...
  if (req.method === 'GET' && url.pathname === '/api/credits') return json(res,200,{ok:true,...ledger.account(accountId),generationCost:1,videoCostEstimate:null});
  if (req.method === 'POST' && url.pathname === '/api/video/prepare') {
    await readBody(req);
    return json(res,501,{ok:false,status:'provider_not_connected',error:'Video generation is not available yet. No credits were charged.'});
  }
  if (req.method === 'POST' && url.pathname === '/api/generate') {
    ...
    const reserved = ledger.reserve(accountId,id,fingerprint,1);
